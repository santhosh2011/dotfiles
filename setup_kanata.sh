...
service_config_path="$HOME/.config/kanata.kbd"
...
<plist version="1.0">
	<dict>
		...
		<array>
			...
			<string>-c</string>
			<string>{service_config_path}</string>
		</array>
		<key>RunAtLoad</key>
		<true/>
		<key>KeepAlive</key>
...
function install {
  ...
  if [[ -z "$service_run_path" || ! -x "$service_run_path" ]]; then
    echo "Error: kanata binary not found on PATH."
    echo "Install it first: cargo install kanata"
    exit 1
  fi
  if [[ ! -f "$service_config_path" ]]; then
    echo "Error: kanata config not found at $service_config_path"
    echo "Run 'stow kanata' from the dotfiles repo first."
    exit 1
  fi
  ...
  | sed \
      ...
      -e "s~{service_config_path}~$service_config_path~g" \
...
function help {
  echo "Usage:
    Installation: kanata-service install
